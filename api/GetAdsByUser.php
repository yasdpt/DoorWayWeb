...
if (isset($_GET['user_id']) && isset($_GET['page'])){
    ...
    $yas_query = "SELECT * FROM ".$tbl_name." WHERE user_id=".$user_id." ORDER BY ".$order_by." DESC LIMIT ".$offset." , ".$records_limit;
    ...
    if( $result )
    {
        ...
        while( $row = mysqli_fetch_array( $result ))
        {
            ...
            $q = " SELECT city FROM tbl_dw_cities WHERE city_id='".$row['city_id']."'";
            $temp1 = mysqli_query($mysqli, $q);
            $temp2 = mysqli_fetch_row( $temp1 );
            $cityname = $temp2[0];
